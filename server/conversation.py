...
def get_conversation_prompt(template: str, current_data: dict, appointment_booked_indicator: str) -> str:
...
def stream_conversation(history: list, appointment_data: dict, client: Client):
    ...
    return bedrock.invoke_model_with_response_stream(
        ...
        body=json.dumps({
            ...
            "max_tokens": 300,
            "system": get_conversation_prompt(CONVERSATION_TEMPLATES[client], appointment_data, APPOINTMENT_BOOKED_INDICATOR[client]),
            "messages": history
        })
    )
