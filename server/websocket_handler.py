...
async def websocket_handler(websocket: WebSocket, client: Client, **kwargs):
    logger.info(f"WebSocket connection received for client: {client}")
    ...
    try:
        async for message in websocket.iter_text():
            ...
            if event == "setup":
                ...
                logger.info(f"Setup event: call_sid={call_sid}, phone_number={phone_number}")
                history = dynamo.get_conversation_history(call_sid)
                logger.info(f"Loaded {len(history)} history entries for call_sid={call_sid}")

            elif event == "prompt":
                user_text = data.get("voicePrompt", "").strip()
                if not user_text:
                    logger.info("Empty voicePrompt received, skipping")
                    continue

                logger.info(f"Prompt event: user_text={user_text!r}")
                is_processing = True

                try:
                    appointment_data = dynamo.get_appointment_data(phone_number, DEFAULT_APPOINTMENT_DATA[client])
                    logger.info(f"Fetched appointment_data for phone_number={phone_number}: {appointment_data}")
                    history.append({"role": "user", "content": [{"text": user_text}]})

                    # Stream conversational response to Twilio
                    logger.info("Starting stream_conversation")
                    ...
                    logger.info(f"Full assistant reply ({token_count} tokens): {assistant_text}")
                    appointment_booked = APPOINTMENT_BOOKED_INDICATOR[client].lower() in assistant_text.lower()
                    logger.info(f"appointment_booked={appointment_booked}")
                    history.append({"role": "assistant", "content": [{"text": assistant_text}]})

                    # Save conversation history
                    logger.info(f"Saving conversation history for call_sid={call_sid} ({len(history)} entries)")
                    dynamo.save_conversation(call_sid, history)

                    # Fire extraction in background
                    logger.info("Firing background extraction task")
                    ...
                    if appointment_booked:
                        ...
                        logger.info(f"Appointment booked. Waiting {speak_time:.1f}s for speech to finish ({word_count} words)")
                        ...
                        if client == Client.PERSONAL and appointment_datetime_start:
                            logger.info(f"Booking Google Calendar appointment at {appointment_datetime_start} for {callers_name}")
                        ...
                        logger.info(f"Sending booking notification for {callers_name} ({phone_number})")
                ...
                logger.info(f"Stop event received for call_sid={call_sid}")
                break

    except Exception as e:
        logger.exception(f"Unhandled error in websocket_handler: {e}")
    finally:
        logger.info(f"Closing WebSocket for call_sid: {call_sid}")
        try:
            await websocket.close()
            logger.info("WebSocket closed cleanly")
        except RuntimeError:
            logger.info("WebSocket already closed (RuntimeError suppressed)")
            pass